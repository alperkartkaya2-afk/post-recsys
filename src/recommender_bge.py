"""
Post recommender variant that uses BGE-M3 for text embeddings and Dinov2 for image embeddings.

Workflow mirrors src/recommender.py:
- load CSVs
- embed text (batched) with BGE-M3 via FlagEmbedding
...
def embed_text(posts: pd.DataFrame, text_model: BGEM3FlagModel, batch_size: int = BATCH_SIZE) -> Dict[str, np.ndarray]:
    """Embed post text with BGE-M3 in batches; skip rows without text."""
    ...
    for start in range(0, len(texts), batch_size):
        ...
        batch_vecs = outputs["dense_vecs"]
        dense_vectors.extend(batch_vecs)

    for post_id, vector in zip(ids, dense_vectors):
        vec = np.asarray(vector, dtype=np.float32)
        embeddings[post_id] = l2_normalize(vec.reshape(1, -1))[0]
    return embeddings
